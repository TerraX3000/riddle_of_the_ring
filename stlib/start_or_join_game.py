...
def initialize_game(game_code, player_code, character, player_name):
...
def initialize_player(game_code, player_code, character, player_name):
    st.session_state["is_initialized"] = True
    st.session_state["points"] = []
...
def run():
    ...
    navbar.run()
    st.header("Enter Player Name")
    name_column, unused = st.columns([2, 10])
    with name_column:
        player_name = st.text_input("Enter Player Name", label_visibility="collapsed")
    if player_name:
        join_game, start_new_game = st.tabs(["Join with Game Code", "Start New Game"])
        with join_game:
                    ...
                    if game_code in valid_game_codes:
                        ...
                        player_code = st.radio(
                            ...
                            options=get_code_options(
                                id=f"{sid}-join_player",
                                excluded_codes=current_player_codes,
                            ),
        ...
        with start_new_game:
            ...
            if st.button("Begin the Quest"):
                ...
                st.query_params.game = game_code
                st.query_params.player = player_code
                initialize_game(game_code, player_code, selected_character, player_name)
                st.rerun()
